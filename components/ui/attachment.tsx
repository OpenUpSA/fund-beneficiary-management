...
// shadcn Attachment, adapted for this project's React 18 and Tailwind 3 setup.
...
const attachmentVariants = cva(
  "group/attachment relative flex w-fit max-w-full min-w-0 shrink-0 flex-wrap rounded-xl border bg-card text-card-foreground transition-colors focus-within:ring-1 focus-within:ring-ring/50 [&:has(>a):hover]:bg-muted/50 [&:has(>button):hover]:bg-muted/50 data-[state=error]:border-destructive/30 data-[state=idle]:border-dashed",
  {
    variants: {
      size: {
        default:
      ...
      orientation: {
        ...
        vertical: "w-24 flex-col [&:has([data-slot=attachment-content])]:w-[7.5rem]",
      },
    },
  }
)
...
function AttachmentTitle({
  ...
  return (
    <span
      ...
      className={cn(
        "block max-w-full min-w-0 truncate font-medium group-data-[state=processing]/attachment:animate-pulse group-data-[state=uploading]/attachment:animate-pulse motion-reduce:animate-none",
        className
      )}
      {...props}
    />
